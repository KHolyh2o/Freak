...
public class MapEditor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (PlayerPrefs.HasKey("AutoLoadSlot"))
        {
            ...
            Debug.Log($"[MapEditor] AutoLoad detected! Loading Slot {slotToLoad}..."); // 로그 추가
    ...
    void PlaceBlock(Vector2Int coord, Vector3 pos)
    {
        ...
        Debug.Log($"[MapEditor] Placing Block ID {currentID} at {coord}. Prefab Rotation: {rot.eulerAngles}"); // 로그 추가
    ...
    {
        string path = Path.Combine(Application.persistentDataPath, $"MapSlot_{slotIndex}.json");
        Debug.Log($"[MapEditor] Loading Map from: {path}"); // 로그 추가
        ...
        foreach (var block in data.blocks)
        {
            ...
            {
                Quaternion rot = prefabs[currentID].transform.rotation;
                Debug.Log($"[MapEditor] Loading Block ID {currentID} at {coord}. Rotation: {rot.eulerAngles}"); // 로그 추가
    ...
    public void SetPlayMode(bool isOn)
    {
        ...
        if (isPlayMode) // [플레이 모드 시작]
        {
            ...
            if (startBoxObj != null)
            {
                ...
                // 플레이어의 회전값도 StartBox의 회전값에 맞춰 설정 (★ 추가된 코드)
    ...
    void ChangeGhost(GameObject prefab)
    {
        ...
        ghostObject.tag = "Untagged"; // 태그 제거 (PlayerController가 찾지 못하게)
